...
const nextConfig = {
    ...
    // ---------------------------------------------------------------
    async headers() {
        const localSupabaseUrl = process.env.NEXT_PUBLIC_SUPABASE_URL?.startsWith("http://127.0.0.1") ? process.env.NEXT_PUBLIC_SUPABASE_URL : null;
        const localRealtimeUrl = localSupabaseUrl?.replace(/^http:/, "ws:");
        ...
        const ContentSecurityPolicy = [
            ...
            "form-action 'self'",
            // Upgrade insecure requests only outside local Supabase validation.
            ...(localSupabaseUrl ? [] : ["upgrade-insecure-requests"]),
        ].join('; ');
