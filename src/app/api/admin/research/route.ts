...
export async function GET(req: NextRequest) {
  ...
  if (target) query = query.ilike("target_name", `%${target}%`);
  if (type && ENTRY_TYPES.has(type)) query = query.eq("type", type);
  if (sourceType && SOURCE_TYPES.has(sourceType)) query = query.eq("source_type", sourceType);
...
export async function POST(req: NextRequest) {
  ...
  if (!rawText && !imageUrl) return NextResponse.json({ error: "Paste content or provide a screenshot URL." }, { status: 422 });
