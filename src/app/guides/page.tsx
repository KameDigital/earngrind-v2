...
function GuideCard({ guide }: { guide: Guide }) {
    ...
    return (
        <Link
            ...
            className="group eg-card flex min-h-[350px] flex-col rounded-none bg-white"
        >
            <div className="relative aspect-[16/8] overflow-hidden bg-slate-900">
                {gameImageUrl ? <Image src={gameImageUrl} alt={guide.games?.name ?? guide.title} fill sizes="(max-width: 640px) 100vw, (max-width: 1024px) 50vw, 33vw" unoptimized={isSvgUrl(gameImageUrl)} className="object-cover transition duration-500 group-hover:scale-105" /> : <div className="flex h-full items-center justify-center bg-[linear-gradient(135deg,#111827,#1e293b)] text-sm font-extrabold tracking-[0.18em] text-white/45">{guide.games ? getInitials(guide.games.name) : "GUIDE"}</div>}
                <div className="absolute inset-0 bg-gradient-to-t from-slate-950/65 via-transparent to-transparent" />
                {guide.games && <span className="absolute bottom-3 left-3 border border-white/20 bg-slate-950/80 px-2 py-1 text-[9px] font-extrabold uppercase tracking-[0.13em] text-white">{guide.games.name}</span>}
                <span className="absolute bottom-3 right-3 bg-rose-500 px-2 py-1 text-[9px] font-extrabold uppercase tracking-[0.13em] text-white">Guide</span>
            </div>
            <div className="flex flex-1 flex-col p-4">
                <h2 className="mb-2 flex-1 text-sm font-extrabold uppercase leading-[1.15] tracking-tight text-[var(--brand-ink)] transition-colors group-hover:text-lime-700">{guide.title}</h2>
                {guide.excerpt && <p className="mb-4 line-clamp-2 text-xs leading-relaxed text-[var(--text-secondary)]">{guide.excerpt}</p>}
                <div className="flex flex-wrap items-center gap-2 border-t border-[var(--border-default)] pt-3">
                    <DifficultyBadge difficulty={guide.difficulty} />
                    {guide.estimated_time && <span className="text-[11px] font-bold uppercase tracking-wide text-[var(--text-tertiary)]">{guide.estimated_time}</span>}
                    {guide.max_payout_usd && <span className="ml-auto text-xs font-extrabold text-lime-600">up to ${guide.max_payout_usd.toFixed(2)}</span>}
                </div>
            </div>
        </Link>
    );
...
function GuideIndexCard({
    ...
    return (
        ...
        >
            <div className="relative aspect-[16/8] overflow-hidden bg-slate-900">
                {imageUrl ? <Image src={imageUrl} alt={label} fill sizes="(max-width: 640px) 100vw, (max-width: 1024px) 50vw, 33vw" unoptimized={isSvgUrl(imageUrl)} className="object-cover transition duration-500 group-hover:scale-105" /> : <div className="flex h-full items-center justify-center bg-[linear-gradient(135deg,#111827,#1e293b)] text-lg font-extrabold tracking-[0.18em] text-white/45">{initials}</div>}
                <div className="absolute inset-0 bg-gradient-to-t from-slate-950/65 via-transparent to-transparent" />
                <span className="absolute bottom-3 left-3 border border-white/20 bg-slate-950/80 px-2 py-1 text-[9px] font-extrabold uppercase tracking-[0.13em] text-white">{label}</span>
                <span className="absolute bottom-3 right-3 bg-rose-500 px-2 py-1 text-[9px] font-extrabold uppercase tracking-[0.13em] text-white">Guide</span>
            </div>
            <div className="flex flex-1 flex-col p-4">
...
function PalmonGuideHub({ imageUrl }: { imageUrl: string | null }) {
    return (
        <section className="overflow-hidden bg-[var(--brand-ink)] p-5 text-white shadow-[var(--shadow-card)] sm:col-span-2 sm:p-6 lg:col-span-3">
            <div className="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
                <div className="flex flex-col gap-4 sm:flex-row sm:items-start">
                    <div className="h-20 w-20 flex-shrink-0 overflow-hidden border border-lime-300/40 bg-lime-50">
                        {imageUrl ? (
                            <Image
                                src={imageUrl}
                                alt="Palmon: Survival"
                    ...
                    <div>
                        <p className="mb-2 text-[10px] font-extrabold uppercase tracking-[0.16em] text-[var(--brand-lime)]">Featured guide cluster</p>
                ...
                <Link
                    ...
                    className="inline-flex w-full items-center justify-center bg-[var(--brand-lime)] px-4 py-3 text-sm font-extrabold text-[var(--brand-ink)] sm:w-auto"
                >
            ...
            <div className="mt-5 grid gap-2 sm:grid-cols-2 lg:grid-cols-4">
                {PALMON_GUIDE_LINKS.map((guide) => (
                    <Link
                        key={guide.href}
                        href={guide.href}
                        className="border border-white/10 bg-white/5 px-4 py-3 text-sm font-bold text-white transition hover:border-lime-300 hover:bg-white/10"
                    >
                        {guide.label}
                    </Link>
                ))}
...
export default async function GuidesPage({
    ...
    return (
        <main className="min-h-screen bg-[#fafbfc] pb-24 pt-8">
            <JsonLd data={schemas} />
            <Container>
                <section className="mb-8 border-b border-[var(--border-default)] px-1 pb-7 pt-2 sm:px-0">
                <nav aria-label="Breadcrumb" className="mb-5 flex flex-wrap items-center gap-2 text-[11px] font-bold uppercase tracking-wide text-[var(--text-tertiary)]">
                    <Link href="/" className="hover:text-lime-700">Home</Link>
                    <span aria-hidden="true">/</span>
                    <span className="text-[var(--brand-ink)]">Guides</span>
                </nav>
                <div>
                    <p className="mb-3 text-[10px] font-extrabold uppercase tracking-[0.18em] text-lime-700">Game guides</p>
                    <h1 className="mb-3 max-w-4xl text-3xl font-extrabold uppercase tracking-[-0.055em] text-[var(--brand-ink)] sm:text-5xl">
                    ...
                    {totalCount > 0 && (
                        <p className="mt-4 inline-flex bg-[var(--brand-ink)] px-2 py-1 text-[10px] font-extrabold uppercase tracking-[0.13em] text-white">
                ...
                <section className="mb-9 grid gap-3 text-sm leading-relaxed text-[var(--text-secondary)] md:grid-cols-3" aria-label="How to use these game offer guides">
                    <div className="border border-[var(--border-default)] bg-white p-4">
                        <h2 className="text-xs font-extrabold uppercase tracking-tight text-[var(--brand-ink)]">What these guides cover</h2>
                        <p className="mt-2">
                    ...
                    <div className="border border-[var(--border-default)] bg-white p-4">
                        <h2 className="text-xs font-extrabold uppercase tracking-tight text-[var(--brand-ink)]">How payouts work</h2>
                        <p className="mt-2">
                    ...
                    <div className="border border-[var(--border-default)] bg-white p-4">
                        <h2 className="text-xs font-extrabold uppercase tracking-tight text-[var(--brand-ink)]">Common mistakes</h2>
                        <p className="mt-2">
                ...
                {page === 1 ? (
                    <section className="mb-8" aria-labelledby="guide-hubs-heading">
                        <div className="mb-4">
                            ...
                            <h2 id="guide-hubs-heading" className="text-xl font-extrabold uppercase tracking-tight text-[var(--brand-ink)]">
                        ...
                        <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
                            {GUIDE_HUB_LINKS.map((link) => (
                                <Link
                                    key={link.href}
                                    href={link.href}
                                    className="border border-[var(--border-default)] bg-white p-4 transition hover:-translate-y-0.5 hover:border-lime-300 hover:shadow-[var(--shadow-card)]"
                                >
