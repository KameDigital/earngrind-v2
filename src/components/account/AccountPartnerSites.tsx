...
export default function AccountPartnerSites({ connections = [], variant = "account" }: PartnerSignupCardsProps) {
    ...
    return <section aria-labelledby={headingId} className={isHomepage ? "" : "account-partners"}>
        <div className="mb-5 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
            ...
            {!isHomepage && <span className="w-fit rounded border border-slate-200 bg-white px-3 py-1.5 text-xs font-extrabold text-slate-800">{connections.length} links opened</span>}
        </div>
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
            {GPT_AFFILIATE_PLATFORMS.map((platform) => {
                ...
                return <article key={platform.slug} className="relative flex min-h-[300px] flex-col overflow-hidden rounded-md border border-slate-200 bg-white p-5 shadow-sm">
                    <div className="absolute inset-x-0 top-0 h-1 bg-gradient-to-r from-emerald-500 to-lime-400" />
                    <div className="flex items-center justify-between gap-3 pt-1">
                        <span className="rounded border border-violet-200 bg-violet-50 px-2 py-1 text-[9px] font-extrabold uppercase tracking-wider text-violet-700">{platform.priority === "primary" ? "Mainstream" : "Partner"}</span>
                        {!isHomepage && <span className="rounded border border-slate-200 bg-slate-50 px-2 py-1 text-[9px] font-extrabold uppercase tracking-wider text-slate-600">{connection ? "Opened" : "Not started"}</span>}
                    </div>
                    <div className="mt-5 flex items-center gap-3">
                        ...
                        />
                        <div><h3 className="font-black text-slate-950">{platform.name}</h3><p className="text-xs text-slate-500">{platform.bestFor}</p></div>
                    </div>
                    <p className="mt-4 text-sm leading-6 text-slate-600">{platform.rewardNote}</p>
