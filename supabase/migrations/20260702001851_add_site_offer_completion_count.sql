alter table public.site_offers
  add column if not exists completion_count integer check (completion_count is null or completion_count >= 0);

create index if not exists idx_site_offers_status_completion_count
  on public.site_offers (status, completion_count desc)
  where completion_count is not null;

drop function if exists public.search_public_offers(
