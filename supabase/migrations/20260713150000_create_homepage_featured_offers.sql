-- A manually curated homepage collection. This is intentionally separate from
-- the offer search/indexing pipeline: inserting an imported offer never adds it
-- to the homepage by itself.
...
CREATE INDEX homepage_featured_offers_public_order_idx
  ON public.homepage_featured_offers (placement, is_active, display_priority, created_at);
...
CREATE POLICY "homepage featured offers public active read"
  ON public.homepage_featured_offers FOR SELECT
  USING (is_active AND (starts_at IS NULL OR starts_at <= now()) AND (expires_at IS NULL OR expires_at > now()));
CREATE POLICY "homepage featured offers admin editor manage"
  ON public.homepage_featured_offers FOR ALL
...
CREATE POLICY "homepage featured settings public read"
  ON public.homepage_featured_settings FOR SELECT USING (true);
CREATE POLICY "homepage featured settings admin editor manage"
