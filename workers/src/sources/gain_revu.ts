...
export async function fetchOffers(): Promise<SourceOffer[]> {
    ...
    const apiKey = requiredEnv("GAIN_REVU_API_KEY");
    const id = requiredEnv("GAIN_REVU_ID");
    const apiUrl = process.env.GAIN_REVU_API_URL ?? DEFAULT_REVU_API_URL;
...
function parseOffer(node: unknown): SourceOffer | null {
    ...
    const externalId = firstString(node.id, node.offerid, node.offer_id, node.campaign_id);
    const rawTitle = firstString(node.title, node.name, node.offer_name);
    const payout = extractPayout(node);
    if (!externalId || !rawTitle || payout <= 0) {
        return null;
    }
    ...
    return {
        ...
        total_payout_raw: payout.toFixed(2),
        task_list: [],
    };
}
...
function extractPayout(payload: Record<string, any>): number {
    const candidates = [payload.payout, payload.amount, payload.reward, payload.usd];
